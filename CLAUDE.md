...
> Version: 2.19.1 · Long-form docs: [docs/README.md](docs/README.md) · Roadmap: [docs/ROADMAP.md](docs/ROADMAP.md) · Release process: [GIT_WORKFLOW.md](GIT_WORKFLOW.md)
...
- `api/pyproject.toml` → `version = "2.19.1"` (then run `uv lock` — `uv.lock` carries it too)
- `web/package.json` → `"version": "2.19.1"`
- `web/src/lib/constants.ts` → `APP_VERSION = "2.19.1"`
- `api/app/main.py` → `VERSION = "2.19.1"`
...
Current baseline: 100 frontend + 260 backend tests, lint/tsc/pyflakes clean. Backend tests patch `app.core.database.get_db` as a FastAPI dependency; a mock-DB round trip in a test means the fixture resets `_shared_mock_db` state (see `tests/conftest.py::_clean_state`).
