...
- Shulker uses yt-dlp which relies on YouTube's internal APIs. These can change without notice and may cause streams or downloads to break temporarily.
- Spotify Client ID and Secret are stored in your server's `.env` file and written there at runtime via `POST /api/settings/spotify`. Do not run a shared Shulker instance without restricting access.
