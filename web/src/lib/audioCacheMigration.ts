...
export function sniffAudioMime(head: Uint8Array): string | null {
  ...
  const ascii = (start: number, end: number): string => {
    let out = ''
    for (let i = start; i < end; i += 1) out += String.fromCharCode(head[i])
    return out
  }
  ...
  if ((sync & 0xffe0) === 0xffe0 && (head[1] & 0x18) !== 0x08) return 'audio/mpeg'
    ...
    for (const entry of all) {
      ...
      if (res && res.ok) {
        try {
          ...
          const finalMime =
            sniffedNew ?? (ct && AUDIO_MIME_RE.test(ct) ? ct.split(';')[0].trim() : null)
